...
\begin{frame}{Vom Sequence Pair zum Packing}
 Aus einem Sequence Pair $(X,Y)$ lassen sich so die Dimensionen eines optimalen Packings ablesen:
 \begin{itemize}
 ...
 Es lässt sich auch eine optimale Position der Rechtecke berechnen. Sei $b$ ein Rechteck, es gelte $X= X_1 b X_2$ und $Y= Y_1 b Y_2$. Dann ist relativ zum rechten Rand:
 \begin{itemize}
...
\begin{algorithm}[H]
	...
	\KwResult{Eine optimale Positionierung der Rechtecke in $x$-Richtung relativ zum rechten Rand}
	\ForEach{Rectangle $b$ in $X$}{
		x\_pos[$b$] = length[$b$]\;
		x\_end = x\_pos[$b$] + $\breite(b)$\;
		\ForEach{Rectangle $c$ in $Y$ after $b$}
		{
			\If{x\_end $>$ length[$c$]}{
				length[$c$] = x\_end\;
			}
...
\begin{figure}
  \centering
  \begin{align*}
  X &= ({\color{blue!40!white} {\color<1-3>{blue} 5}}, {\color{green!40!white} {\color<4-5>{green}4}}, {\color{orange!40!white} {\color<6-10>{orange}3}} ,{\color{red!40!white} 2} ,{\color{purple!40!white} 1})\\
  Y &=  ({\color{orange!40!white} 3} ,{\color{red!40!white} {\color<7>{red}2}}, {\color{blue!40!white} \color<8>{blue}5}, {\color{green!40!white} {\color<2,9>{green}4}} ,{\color{purple!40!white} {\color<3,5,10>{purple}1}})
  \end{align*}
  \begin{tikzpicture}
  \filldraw[blue,fill=blue!15!white] (3,0) rectangle node{5} +(3,2);
  \filldraw[green,fill=green!15!white, visible on = <2->] (2,0) rectangle node{4} +(1,1);
  \filldraw[purple,fill=purple!15!white, visible on = <3-4>] (1,1) rectangle node{1} +(2,2) ;  
  \filldraw[purple,fill=purple!15!white, visible on = <5->] (0,0) rectangle node{1} +(2,2) ;  
  \filldraw[orange,fill=orange!15!white, visible on = <6->] (5,2) rectangle node{3} +(1,2);
  \filldraw[red,fill=red!15!white, visible on = <7->] (3,2) rectangle node{2} +(2,1);
...
\begin{frame}[fragile]{Vom Sequence Pair zum Packing}
 \begin{lstlisting}[basicstyle=\scriptsize]
...
auto cur_node = cur_seqs.insert({ y_index + 1, 0 }).first;
positions[pack_index] = std::prev(cur_node)->second;
pos cur_seq_length = positions[pack_index] +
    pack.get_rect(pack_index).get_dimension(dim);
cur_seqs[y_index + 1] = cur_seq_length;
//We go forward until we no longer need to delete nodes
while (std::next(cur_node) != cur_seqs.end() && 
    std::next(cur_node)->second < cur_seq_length)
{
