...
void graph::augpath(path p, weight w)
{
  ...
  if (w == 0)
  {
    w = std::min(_list[p.front().from].demand, -_list[p.back().to].demand);
    for (auto const &e: p)
    {
      if (e.reverse)
      {
  ...
  assert(w <= _list[p.front().from].demand);
  assert(w <= -_list[p.back().to].demand);
  _list[p.front().from].demand -= w;
  _list[p.back().to].demand += w;
}
...
path graph::compute_shortest_path(size_t from)
{
  ...
  while(prev_nodes[cur] != _invalid_index)
  {
    ...
    cur = prev_nodes[cur];
    path.push_front(*std::find_if(_list[cur].adjacent.begin(), _list[cur].adjacent.end(), [t](const edge &e)
    {
      return e.to == t;
    }));
